        ...
        {orders.length > 0 ? (
          orders.map((o) => (
            <tr key={o.id} className={trClass}>
              ...
              <td className={`${tdClass} whitespace-nowrap`}>{o.totalAmount.toLocaleString()}</td> 
              <td className={tdClass}>
                <button className={editButtonClass} onClick={() => handleEdit(o)}>✏️</button>
                <button className={deleteButtonClass} onClick={() => handleDelete(o.id)}>🗑️</button>
              </td>
            </tr>
          ))
        ) : (
          <tr><td colSpan="6" className="py-4 px-3 text-center text-gray-500">Không tìm thấy đơn hàng</td></tr>
        )}
      </tbody>
    </table>
  );
