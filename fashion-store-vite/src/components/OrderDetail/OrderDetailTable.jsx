...
export default function OrderDetailTable({
  orderDetails,
  // handleSort, // <- Đã xóa theo logic file
  // sortField,
  // sortOrder,
  handleEdit,
  handleDelete,
        ...
        {orderDetails.length > 0 ? (
          orderDetails.map((d) => (
            <tr key={d.id} className={trClass}>
              ...
              <td className={tdClass}>{d.orderId}</td>
              <td className={tdClass}>{d.productId}</td>
              <td className={tdClass}>{d.quantity}</td>
              <td className={`${tdClass} whitespace-nowrap`}>{d.unitPrice.toLocaleString()} đ</td>
              <td className={tdClass}>
                <button className={editButtonClass} onClick={() => handleEdit(d)}>
                  ✏️
                </button>
                <button
          ...
          <tr>
            <td colSpan="6" className="py-4 px-3 text-center text-gray-500">Không có chi tiết đơn nào</td>
          </tr>
        )}
      </tbody>
    </table>
