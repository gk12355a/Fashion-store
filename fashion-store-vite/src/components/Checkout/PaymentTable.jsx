        ...
        {payments.length > 0 ? (
          payments.map((p) => (
            <tr key={p.id} className={trClass}>
              ...
              <td className={`${tdClass} whitespace-nowrap`}>{p.amount.toLocaleString('vi-VN')}</td>
              <td className={`${tdClass} whitespace-nowrap`}>{formatDateTime(p.paymentDate)}</td>
          ...
          <tr><td colSpan="7" className="py-4 px-3 text-center text-gray-500">Không tìm thấy thanh toán</td></tr>
        )}
      </tbody>
    </table>
  );
