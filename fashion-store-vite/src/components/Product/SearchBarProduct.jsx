// src/components/Product/SearchBarProduct.jsx
import React from 'react';

// Icon Kính lúp (để nhúng trực tiếp)
const SearchIcon = () => (
    ...
    <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4 font-['Helvetica_Neue',_'Arial',_sans-serif]">
      ...
      <div className="relative w-full md:flex-grow">
        ...
        <input
          ...
          onBlur={onBlur} // Giữ lại onBlur cho autocomplete
        ...
        {suggestions && suggestions.length > 0 && (
          <ul className="absolute top-full left-0 right-0 z-10 mt-1 bg-white border-2 border-gray-700/20 rounded-lg shadow-xl max-h-60 overflow-y-auto">
            {suggestions.map((suggestion, index) => (
              <li
                key={index} // Giả sử suggestion là string, nếu là object thì dùng suggestion.id
                className="px-4 py-3 hover:bg-gray-700/5 cursor-pointer font-medium text-gray-800 border-b border-gray-100 last:border-b-0 transition duration-200"
                onMouseDown={() => onSuggestionClick(suggestion)} // Dùng onMouseDown
              >
                {suggestion}
              </li>
            ))}
