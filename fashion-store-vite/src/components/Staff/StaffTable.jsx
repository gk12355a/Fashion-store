        ...
        {staffs.length > 0 ? (
          staffs.map((s) => (
            <tr key={s.id} className={trClass}>
              ...
              <td className={`${tdClass} text-left pl-3`}>{s.name}</td>
              <td className={tdClass}>{s.position}</td>
              <td className={`${tdClass} whitespace-nowrap`}>{s.salary.toLocaleString('vi-VN')}</td>
              <td className={tdClass}>{s.workShift}</td>
              <td>
                <button className={editButtonClass} onClick={() => handleEdit(s)}>✏️</button>
                <button className={deleteButtonClass} onClick={() => handleDelete(s.id)}>🗑️</button>
              </td>
            </tr>
          ))
        ) : (
          <tr><td colSpan="6" className="py-4 px-3 text-center text-gray-500">Không tìm thấy nhân viên</td></tr>
        )}
      </tbody>
    </table>
  );
